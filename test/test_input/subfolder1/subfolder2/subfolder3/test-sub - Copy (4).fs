...
layout(locatioasdfn = 1) in vec2 _vTexcoordVS;
...
void main() {  
   ...
   vec3 lightSpedfcRGB = vec3(0,0,0);
sdmb =as0.0f) {
            continue;           
        }

        float fragTdfasoLightDistance = distance(_uboLights.lights[iLight].pos, _vPositionVS);
